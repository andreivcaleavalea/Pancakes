...
public class FriendshipServiceTests
{
    ...
    [Fact]
    public async Task GetUserFriends_Returns_FriendDto_List()
    {
    ...
    [Fact]
    public async Task Send_And_Accept_And_Reject_Work()
    {
        var s = CreateService(out var repo, out var users, out var current);
    ...
    [Fact]
    public async Task SendFriendRequest_Throws_On_Self_Or_Existing_Or_Missing_Receiver()
    {
        var s = CreateService(out var repo, out var users, out var current);
        await FluentActions.Invoking(() => s.SendFriendRequestAsync("u1", "u1")).Should().ThrowAsync<ArgumentException>();

        repo.Setup(r => r.GetFriendshipAsync("u1", "u2")).ReturnsAsync(new Friendship());
        await FluentActions.Invoking(() => s.SendFriendRequestAsync("u1", "u2")).Should().ThrowAsync<InvalidOperationException>();
        ...
        await FluentActions.Invoking(() => s.SendFriendRequestAsync("u1", "u3")).Should().ThrowAsync<ArgumentException>();
    }

    [Fact]
    public async Task Accept_Reject_Remove_Throw_On_Invalid_State()
    {
        var s = CreateService(out var repo, out var users, out var current);
        var id = Guid.NewGuid();
        repo.Setup(r => r.GetByIdAsync(id)).ReturnsAsync((Friendship?)null);
        await FluentActions.Invoking(() => s.AcceptFriendRequestAsync(id, "u2")).Should().ThrowAsync<ArgumentException>();
        ...
        await FluentActions.Invoking(() => s.AcceptFriendRequestAsync(id, "u2")).Should().ThrowAsync<UnauthorizedAccessException>();
        ...
        await FluentActions.Invoking(() => s.AcceptFriendRequestAsync(id, "u2")).Should().ThrowAsync<InvalidOperationException>();
        ...
        await FluentActions.Invoking(() => s.RejectFriendRequestAsync(fr.Id, "u2")).Should().ThrowAsync<ArgumentException>();
        ...
        await FluentActions.Invoking(() => s.RejectFriendRequestAsync(fr.Id, "u2")).Should().ThrowAsync<UnauthorizedAccessException>();
        ...
        await FluentActions.Invoking(() => s.RejectFriendRequestAsync(fr.Id, "u2")).Should().ThrowAsync<InvalidOperationException>();

        repo.Setup(r => r.GetFriendshipAsync("a", "b")).ReturnsAsync((Friendship?)null);
        await FluentActions.Invoking(() => s.RemoveFriendAsync("a", "b")).Should().ThrowAsync<ArgumentException>();
    }

    [Fact]
    public async Task RemoveFriend_And_Check_AreFriends()
    {
        var s = CreateService(out var repo, out var users, out var current);
    ...
    [Fact]
    public async Task Http_Wrappers_Cover_Branches()
    {
        var s = CreateService(out var repo, out var users, out var current);
        var http = new DefaultHttpContext();

        // GetFriends unauthorized
