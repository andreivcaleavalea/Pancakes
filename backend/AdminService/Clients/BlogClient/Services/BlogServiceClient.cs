...
namespace AdminService.Clients.BlogClient.Services
{
    public class BlogServiceClient : IBlogServiceClient
    {
        ...
        public BlogServiceClient(HttpClient httpClient, ILogger<BlogServiceClient> logger, IConfiguration configuration, IServiceJwtService serviceJwtService)
        {
            ...
            var baseUrl = Environment.GetEnvironmentVariable("BLOG_SERVICE_URL") 
                           ?? Environment.GetEnvironmentVariable("BLOG_API_BASE_URL") 
                           ?? Environment.GetEnvironmentVariable("BLOG_API_URL") 
                           ?? Environment.GetEnvironmentVariable("BLOG_SERVICE_BASE_URL") 
                           ?? "http://localhost:5001";
