...
const SectionSettingsPopover: React.FC<SectionSettingsPopoverProps> = ({
  ...
  // Helper function to get effective value (handles both undefined and empty strings)
  const getEffectiveValue = (newValue: any, currentValue: any) => {
    // If newValue is null or undefined, use currentValue
    if (newValue === null || newValue === undefined) {
      return currentValue || "";
    }
    // For strings, treat empty string as "not set" and use current value
  ...
  // Real-time change handler with proper typing
  const handleChange = (field: string, value: string | number | boolean) => {
    const currentValues = form.getFieldsValue();
    const updatedValues = { ...currentValues, [field]: value };
    
    // CRITICAL FIX: Always preserve both color and pattern, only update the field that changed
    const finalBackground = {
      color: field === 'backgroundColor' 
        ? (value as string || "")  // Use the new color value
        : (currentAdvancedSettings.background.color || ""),  // Keep existing color
      pattern: field === 'backgroundPattern'
        ? (value as string || "none")  // Use the new pattern value  
        : (currentAdvancedSettings.background.pattern || "none"),  // Keep existing pattern
      opacity: field === 'backgroundOpacity'
        ? (value as number || 1.0)  // Use the new opacity value
        : (currentAdvancedSettings.background.opacity || 1.0)  // Keep existing opacity
    };
  ...
  // Convert numeric values back to strings for backend
